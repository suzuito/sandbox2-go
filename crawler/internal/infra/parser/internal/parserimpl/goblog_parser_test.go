...
func TestGoBlogParser(t *testing.T) {
	ctx := context.Background()
	parser := GoBlogParser{
		BaseURLGoBlog: "https://go.dev",
	}
	data, err := parser.Do(ctx, bytes.NewBufferString(bodyGolangParserTest), nil)
	if err != nil {
		t.Fail()
		return
	}
	assert.Equal(t, crawler.ParserID("goblog"), parser.ID())
