...
func TestNewDiscordBlogFeedNotifier(t *testing.T) {
	testCases := []struct {
		desc                     string
	...
	}{
		{
			desc: "Success",
			inputDef: &notifier.NotifierDefinition{
				ID:       notifier.NotifierID("discordblogfeed"),
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			n, err := NewDiscordBlogFeedNotifier(tC.inputDef, tC.inputSetting)
			test_helper.AssertError(t, tC.expectedError, err)
			if err == nil {
				assert.Equal(t, tC.expectedDiscordChannelID, n.(*notifierimpl.DiscordBlogFeedNotifier).DiscordChannelID)
			}
		})
	}
}
